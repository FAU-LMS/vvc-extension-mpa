# Viewport-adaptive VTM extension

This is a quick documentation on the viewport-adaptive VTM extension. For general information
and instructions on VTM, please read the section
[VTM reference software for VVC](#vtm-reference-software-for-vvc) provided below.

The viewport-adaptive extension provides the following additional command line options for the
encoder:
* **VA**: Enable viewport-adaptive inter prediction (0:off, 1:on)
* **VAMVP**: Enable viewport-adaptive motion vector prediction (0:off, 1:on)
* **VAOffset4x4**: Offset of mv reprojection calculation within 4x4 subblocks (0:0, 1:1, 2:2, 3:3, 4:1.5)
* **Projection**: Projection function for VA (0:equisolid, 1:calibrated)
* **FocalLength**: Focal length [px] for projection (fixed precision 16.16)
* **OpticalCenterX**: Optical center x [px] for projection (fixed precision 16.16)
* **OpticalCenterY**: Optical center y [px] for projection (fixed precision 16.16)
* **CalibratedCoefficients**: Coefficients [px] for calibrated projection function (comma-separated list, fixed precision 16.16)
