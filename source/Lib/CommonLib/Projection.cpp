...
Array3TCoord PerspectiveProjection::toSphere(Array2TCoord cart2D, bool virtualImagePlane) const {
  const TCoord cart2DX = cart2D.x() - m_opticalCenter.x();
  const TCoord cart2DY = cart2D.y() - m_opticalCenter.y();
  const Array2TCoord polar = CoordinateConversion::cartesianToPolar({cart2DX, cart2DY});
...
PerspectiveProjection::fromSphere(Array3TCoord cart3D) const {
  const TCoord cart3DRotX = cart3D.y();
  const TCoord cart3DRotY = -cart3D.z();
  const TCoord cart3DRotZ = -cart3D.x();
  const Array3TCoord spherical = CoordinateConversion::cartesianToSpherical({cart3DRotX, cart3DRotY, cart3DRotZ});
...
ArrayXXTCoordPtrTriple EquirectangularProjection::toSphere(ArrayXXTCoordPtrPair cart2D) const {
  ArrayXXTCoordPtr cart2DX = std::make_shared<ArrayXXTCoord>(*std::get<0>(cart2D));
  ArrayXXTCoordPtr cart2DY = std::make_shared<ArrayXXTCoord>(*std::get<1>(cart2D));
...
Array3TCoord EquirectangularProjection::toSphere(const Array2TCoord &cart2D) const {
  const TCoord sphericalPhi = -(cart2D.x() / TCoord(m_resolution.width)) * TCoord(2) * TCoord(M_PI);
  const TCoord sphericalTheta = (cart2D.y() / TCoord(m_resolution.height)) * TCoord(M_PI);
...
ArrayXXTCoordPtrPair EquirectangularProjection::fromSphere(ArrayXXTCoordPtrTriple cart3D) const {
  ...
  ArrayXXTCoordPtr cart2DX = std::make_shared<ArrayXXTCoord>(-(*sphericalPhi / (TCoord(2) * TCoord(M_PI))) * TCoord(m_resolution.width));
  ArrayXXTCoordPtr cart2DY = std::make_shared<ArrayXXTCoord>((*sphericalTheta / TCoord(M_PI)) * TCoord(m_resolution.height));
  return {cart2DX, cart2DY};
}
...
Array2TCoord EquirectangularProjection::fromSphere(const Array3TCoord &cart3D) const {
  ...
  const TCoord cart2DX = -(sphericalPhi / (TCoord(2) * TCoord(M_PI))) * TCoord(m_resolution.width);
  const TCoord cart2DY = (sphericalTheta / TCoord(M_PI)) * TCoord(m_resolution.height);
  return {cart2DX, cart2DY};
}
