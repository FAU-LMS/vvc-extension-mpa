...
void MVReprojection::init(const Projection *projection, const Size &resolution, TCoord offset4x4) {
  m_projection = projection;
  m_resolution = resolution;
  m_offset4x4 = offset4x4;
  m_perspective = PerspectiveProjection(projection->focalLength(), Array2TCoord(0, 0));
  m_lastViewport = INVALID;
  fillCache();
}
...
MVReprojection::toPerspective(ArrayXXTCoordPtrPair cart2DProj, Viewport viewport) const
{
  if (viewport == CLASSIC) {
    return { cart2DProj,
...
std::tuple<Array2TCoord, bool> MVReprojection::toPerspective(Array2TCoord cart2DProj, Viewport viewport) const {
  if (viewport == CLASSIC) {
...
ArrayXXTCoordPtrPair MVReprojection::toProjection(ArrayXXTCoordPtrPair cart2DPers, Viewport viewport,
...
Array2TCoord MVReprojection::toProjection(Array2TCoord cart2DPers, Viewport viewport, bool virtualImagePlane) const {
  if (viewport == CLASSIC) {
    return cart2DPers;
  }
...
ArrayXXFixedPtrPair
MVReprojection::reprojectMotionVector4x4(const Position &position, const Size &size, const Mv &motionVector,
...
Mv MVReprojection::motionVectorInDesiredViewport(const Position &position, const Mv &motionVectorOrig,
  ...
  if (vipMovedDesired != vipDesired && viewportDesired != CLASSIC) {
